...
templates (civil + criminal), and sixteen State exemplars as MCP tools that
...
mcp = FastMCP("wolfgang-district-court-drafting")
...
CASE_TYPE_DESCRIPTIONS: dict[str, str] = {
    "specific-relief-suit": "Suit under the Specific Relief Act 1963 — specific performance, injunction, declaration",
    "plaint-ws": "Plaint and Written Statement under CPC 1908 (Order 6, 7, 8)",
    "petition": "Petition before the District Court (generic)",
    "application": "Interlocutory application under CPC Order 39 / Order 1 / Order 41 etc.",
    "pleadings": "General pleadings template",
    "criminal-complaint": "Criminal complaint under BNSS 2023 (replacing CrPC 1973)",
    "criminal-revision": "Criminal revision before the Sessions Court / District Judge",
    "bail": "Bail application under BNSS Section 480 / 483",
    "anticipatory-bail": "Anticipatory bail under BNSS Section 482",
    "bnss-313-statement": "BNSS Section 313 / 351 statement of accused",
}
...
def _list_states_internal() -> list[str]:
    """Derive available states from state-config/exemplars/ folder."""
...
specific-relief-suit, plaint-ws, petition, application, pleadings, criminal-complaint, criminal-revision, bail, anticipatory-bail, bnss-313-statement.
...
def get_pleading_base() -> str:
    """Get the shared District pleading base structure.

    Returns the universal District Court pleading skeleton (Cause Title to
    Plaint / Complaint Frame to Issues to Prayer to Verification) and the
    cross-cutting drafting discipline (citation, court-fee computation,
    Vakalatnama format, AI-fabricated-citation risk).
    """
    base_dir = SKILLS_DIR / "_district_pleading_base"
...
def list_states() -> dict:
    """List the State exemplars available for District Court drafting.

    Returns the list of State (or grouped-State) exemplars this connector
    bundles. Each exemplar captures the local Civil Manual conventions,
    Court-Fees Act values, Vakalatnama format, and language requirements
    for that jurisdiction. Call get_state_exemplar(state) to fetch the content.
    """
    states = _list_states_internal()
    return {
        "states": states,
        "count": len(states),
        "note": "Call get_state_exemplar(state) for the per-jurisdiction content. State names use lowercase hyphenated form (e.g., 'maharashtra', 'tamil-nadu', 'andhra-pradesh-telangana').",
    }
...
def get_state_exemplar(state: str) -> str:
    """Get the State-specific exemplar for a District Court jurisdiction.

    Returns the State-specific Civil Manual conventions, Court-Fees Act
    schedule values, Vakalatnama format, local-language requirements, and
    any State-specific procedural specialities (e.g., Bombay City Civil
    Court Act for Mumbai, Tamil Nadu Court-Fees and Suits Valuation Act).

    Args:
        state: Lowercase hyphenated state name. Use list_states() to discover
               available identifiers (e.g., 'maharashtra', 'karnataka',
               'andhra-pradesh-telangana').
    """
    available = _list_states_internal()
    if state not in available:
        return (
            f"Error: no exemplar found for state '{state}'. "
            f"Available: {', '.join(available)}. "
            "Call list_states() to discover available identifiers."
        )
...
def save_draft_as_docx(markdown_text: str, output_path: str) -> dict:
    ...
    pandoc. If the District pleading base ships a reference.docx, it is
    applied for trial-court formatting conventions.
    ...
    reference_docx_candidates = [
        SKILLS_DIR / "_district_pleading_base" / "reference.docx",
        SKILLS_DIR / "_district_pleading_base" / "REFERENCE.docx",
    ]
